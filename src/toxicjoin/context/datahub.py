...
The live adapter validates an explicit logical-name-to-URN manifest, reads entities
and schema fields through the official MCP server, classifies fields from controlled
tags or glossary terms, and materializes the existing ``FixtureCatalog`` model. The
same deterministic policy path is therefore used by fixture and live modes.
"""
...
class DataHubAssetMap(StrictModel):
    ...
    def validate_manifest(self) -> "DataHubAssetMap":
        ...
        if any(not name.strip() for name in self.datasets):
    ...
    def from_path(cls, path: str | Path) -> "DataHubAssetMap":
        ...
        try:
            ...
            raise ValueError(
                f"unable to read DataHub asset manifest: {manifest_path}"
            ) from exc
        except json.JSONDecodeError as exc:
            raise ValueError(
                f"invalid DataHub asset manifest JSON: {manifest_path}"
            ) from exc
        ...
        for logical_name, urn in self.asset_map.datasets.items():
            ...
            for field in fields:
                ...
                if field_path in normalized_fields:
                    raise DataHubMetadataError(
                        f"duplicate field path in DataHub schema: "
                        f"{logical_name}.{field_path}"
                    )
...
def _normalize_field(field: dict[str, Any]) -> FixtureField:
    ...
    if not categories:
        ...
        raise DataHubMetadataError(
            f"conflicting sensitivity categories on field "
            f"{_field_path(field)}: {rendered}"
        )

...
operation. The stable MCP Python SDK v1 is loaded lazily so fixture mode remains
lightweight unless ``requirements-datahub.txt`` is installed.
"""
    ...
    async def __aenter__(self) -> Self:
        try:
            ...
            raise DataHubMcpDependencyError(
                "install the MCP integration with: "
...
def _find_document_urn(value: Any) -> str | None:
    if isinstance(value, str):
        return value if value.startswith("urn:li:document:") else None
    if isinstance(value, dict):
        for key in ("urn", "documentUrn", "document_urn"):
            found = _find_document_urn(value.get(key))

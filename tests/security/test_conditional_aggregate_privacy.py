...
def _filtered_probe(threshold: float, *, customer_id: str = "cust_0011") -> str:
    return f"""
    SELECT
        COUNT(r.churn_score) FILTER (
...
def test_filtered_count_is_classified_as_conditional_aggregate() -> None:
    plan = analyze_sql(_filtered_probe(0.60))
    probe = _exposure(plan, "probe")
...
def test_outer_k_threshold_does_not_authorize_protected_conditional_count() -> None:
    plan, decision = _decision(_conditional_probe(0.60))
...
def test_outer_k_threshold_does_not_authorize_filtered_count_oracle() -> None:
    plan, decision = _decision(_filtered_probe(0.60))
...
def test_filtered_threshold_variants_are_distinct_cohort_inputs() -> None:
    from toxicjoin.disclosure import build_composition_metadata, build_semantic_release

    first = _filtered_probe(0.60)
    second = _filtered_probe(0.70)
    catalog = default_fixture_catalog()
    first_semantic = build_semantic_release(catalog, analyze_sql(first))
    second_semantic = build_semantic_release(catalog, analyze_sql(second))
    secret = b"x" * 32

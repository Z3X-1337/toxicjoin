...
- the bundled judge SPA is not mounted;
- `/assets/*` from the bundled judge distribution is not mounted even when a valid `web_dist` exists in the image;
- the service root returns only the product name and does not disclose package version or documentation location.

The unauthenticated fixture/judge deployment intentionally retains docs, demo scenarios, benchmark evidence, and the bundled judge interface for evaluation UX.
...
Production deployments must terminate TLS at a trusted ingress/reverse proxy or ASGI server and must configure proxy/forwarded-header trust so the application cannot be tricked into treating untrusted HTTP as trusted HTTPS. HSTS is therefore conditional on the ASGI scheme that reaches the application. ToxicJoin itself does not interpret a client-supplied `X-Forwarded-Proto` header as proof that the request is HTTPS.
...
- **Demo/evidence exposure:** curated demo scenarios, benchmark evidence, judge SPA, and judge static assets are absent from authenticated/LIVE surfaces.
- **Host-header abuse:** restricted surfaces reject hosts outside an explicit allowlist.
- **Forwarded-protocol spoofing at the app boundary:** an arbitrary `X-Forwarded-Proto: https` header does not by itself enable HSTS or establish HTTPS trust.
...
- an explicit judge distribution containing marker content cannot be served from the restricted root or `/assets/*`;
...
- wildcard, empty, URL-shaped, path-bearing, whitespace-bearing, and oversized Host allowlist configuration fails closed;
- genuine HTTPS ASGI requests emit HSTS;
- client-supplied `X-Forwarded-Proto: https` over an HTTP ASGI request does not emit HSTS;
...
- TLS termination, certificate lifecycle, cipher policy, and trusted proxy-header configuration remain deployment responsibilities. A deployment that intentionally converts trusted proxy headers into the ASGI scheme must configure that trust at the ingress/server layer.

...
- credential/session identifiers as audit metadata only;
- governed subject namespace metadata;
- receipt ID;
- SHA-256 query identity, never raw SQL;
- policy version;
...
Raw warehouse rows, result values, SQL text, SQL literals, API keys, and task prompts are intentionally absent.
...
`credential_id` and `session_id` are intentionally excluded from the scope key. Rotating an API credential or session must not reset cumulative privacy history.
...
Output aliases/names are retained for audit but excluded from the semantic fingerprint so alias renaming cannot create a fresh privacy identity.

P2-A still stores a SHA-256 hash of the exact query for audit/idempotency, but raw SQL and literal values are not stored.
...
- owner-only (`0600`) database creation/permissions;
- symbolic-link target rejection;
- fail-closed validation of persisted payloads against indexed columns and hashes.

The serialized writer transaction is intentionally chosen so P2-B can perform history read + composition evaluation + accepted append under one writer transaction. P2-A itself exposes only append/read/verify operations and therefore does not yet close a time-of-check/time-of-use race around privacy authorization.

## Threats reduced

- **Credential rotation bypass:** credential IDs do not partition privacy history.
- **Session rotation bypass:** session IDs do not partition privacy history.
...
- **Alias-based semantic identity bypass:** output aliases are excluded from semantic fingerprinting.
...
- **Sensitive ledger content:** raw SQL, literals, rows, result values, API keys, and task prompts are not persisted.
...
- alias changes preserving the semantic fingerprint;
...
- raw SQL and literal marker strings never appearing in the SQLite file;
...
- **Predicate/cohort relation reasoning is not complete.** P2-A captures governed referenced columns and exact-query hashes but intentionally does not claim that it can infer set-difference relationships. P2-B/P2-C must add the controlled-query/composition model and adversarial sequence corpus.

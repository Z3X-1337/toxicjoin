...
| The hosted site is immediately testable | Pending Chromium evidence and public URL | “The hosted site is a deterministic Replay with explicit disclosure; Docker/FastAPI is the executable product path.” | Live DataHub writes, live DuckDB execution, or dynamic API results on the static site |
...
5. The live DataHub evidence used an ephemeral DataHub OSS environment in GitHub Actions.
6. ToxicJoin is not a legal compliance certification, differential privacy system, or replacement for organization-specific governance review.

...
The live DataHub evidence is retained as exact evidence for the revision on which it was produced. Later unrelated vNext hardening is not retroactively described as having rerun that exact live integration unless an applicable Live DataHub gate did so.
...
Phase 16 proved that two replicas using separate local SQLite files can partition cumulative privacy history. ToxicJoin now rejects declared multi-replica use of that local state rather than pretending it is globally authoritative.
...
The repository preserves the earlier PR #68/#70 release evidence, production-image black-box report, and hosted deterministic replay provenance.

Those artifacts remain valuable evidence, but their historical SHAs must not be presented as the current repository identity.

The hosted Vercel experience is explicitly a **Deterministic Replay** and is not represented as a build of current `main` or as a live DataHub mutation environment.
...
The current Devpost write-up predates the final vNext hardening phases. Before submission it must be rewritten so that:
...
- DataHub write-back + fresh read-back is prominent;
...
The optimization target is now **judge comprehension + reproducibility + evidence integrity**, not additional feature count.

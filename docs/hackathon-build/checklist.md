...
- [ ] **12. Complete security hardening and independent external validation**
  Security plan: `docs/security/hardening-roadmap.md`.
  Why this is release-blocking: white-box review, synthetic-fixture pentesting, external blind research, and dependency/SAST audits identified real gaps that are not covered by the original 30-case benchmark or the 144-case metamorphic suite. These include request-local privacy composition, the Policy v0.1.0 external misses, output-exposure semantics, decision/execution binding, API access control/resource limits, DataHub edited-field normalization, and supply-chain reproducibility.
  Required implementation sequence: P0 correctness/execution authority -> P1 identity/resource controls -> P2 cumulative disclosure state -> P3 DataHub freshness/least privilege -> P4 supply-chain gates -> P5 release validation.
  Acceptance: all confirmed P0/P1 release blockers are fixed; cumulative-disclosure tests pass; the same frozen external blind workload is rerun without retroactive relabeling; no declared high-risk case reaches an unsafe effective ALLOW; exact authorization binding prevents decision/SQL/context drift; real DataHub live evidence is green; dependency/SAST/container gates are green on the exact release candidate; and the security threat-model delta checklist becomes mandatory for future sensitive changes.
  Evidence policy: negative baselines are retained. A failed security experiment is never rewritten into a passing fixture merely to make CI green.
...
- [ ] **13. Complete Devpost handoff**
  ...
  Verify: Follow `docs/security/hardening-roadmap.md`, `docs/judge-testing.md`, `docs/evidence/hosted-replay.md`, and `docs/submission/owner-review-index.md`, then complete `docs/submission/review-checklist.md`.

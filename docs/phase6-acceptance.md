...
- Browser tests use the hardened production container, not `vite`, a mock server, generated UI, or static results as a substitute.
...
- An induced `execute-safe` 503 produces the real failure disclosure and retry control.
- An unavailable bootstrap API produces an explicit Render/API failure, a reconnect control, no synthetic receipt, and no `execute-safe` request.
...
- The hosting provider and Devpost are not mutated.
